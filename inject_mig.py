...
# Insert inside export const AppLayout = ({ children }: AppLayoutProps) => {
match = re.search(r'export const AppLayout\s*=\s*\([^)]*\)\s*=>\s*\{', content)
if match:
    insert_idx = match.end()
    new_content = content[:insert_idx] + "\n" + migration_code + content[insert_idx:]
    with io.open(file, 'w', encoding='utf8') as f:
        f.write(new_content)
    print("Injected migration code.")
else:
    print("Could not find AppLayout component.")
